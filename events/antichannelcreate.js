...
module.exports = async (client) => {
    client.on('channelCreate', async (channel) => {
        ...
        const auditLogs = await channel.guild
            .fetchAuditLogs({ limit: 2, type: AuditLogEvent.ChannelCreate })
            .catch((_) => { })
        ...
        await client.db
            ...
            .then(async (data) => {
                ...
                if (antinuke.antinuke !== true && !antinuke.antichannelcreate) return
                if (data) {
                    if (data.chcr) return
                }
                ...
                if (panic) {
                    ...
                    if (!action || action === 'ban') {
                        try {
                            if (executor.bot) {
                                    ...
                                    .forEach(async (r) => {
                                            try {
                                                if (r.permissions.has("ManageChannels") || r.permissions.has("Administrator")) {
                                                    await r.setPermissions([], 'Panic Mode | Anti Channel Create | Not Whitelisted').catch((_) => { })
                                                }
                                ...
                                await client.util.FuckYou(executor, 'Panic Mode | Anti Channel Create | Not Whitelisted').catch((_) => { })
                                } 
                            } else {
                                await channel.delete()
                                await Promise.all([
                                    executor.guild = channel.guild,
                                    perms.map(async role => await role.setPermissions([], 'Panic Mode | Anti Channel Create | Not Whitelisted')),
                                    await member1.roles.remove(perms, `Panic Mode | Anti Channel Create | Not Whitelisted`),
                                    await client.util.FuckYou(executor, 'Panic Mode | Anti Channel Create | Not Whitelisted'),
                                ])
                            }
                        ...
                        try {
                            ...
                            if (executor.bot) {
                                ...
                                if (ok) {
                                    ...
                                    managed.map(async x => await x.setPermissions([], 'Panic Mode | Anti Channel Create | Not Whitelisted')).catch((_) => { })
                                    await member1.roles.add(role.id, `Panic Mode | Anti Channel Create | Not Whitelisted`).catch((_) => { })
                                }
                            } else {
                                await channel.delete().catch((_) => { })
                                await Promise.all([
                                    executor.guild = channel.guild,
                                    await member1.roles.set([role.id], `Panic Mode | Anti Channel Create | Not Whitelisted`),
                                    perms.map(async role => await role.setPermissions([], 'Panic Mode | Anti Channel Create | Not Whitelisted')),
                                ])
                            }
                            await client.util.sleep(2000)
                    ...
                    try {
                        executor.guild = channel.guild
                        await client.util
                            .FuckYou(executor, 'Channel Create | Not Whitelisted')
                            .catch((err) => null)
                        await channel.delete().catch((_) => { })
                    } catch (err) {
                        console.error(err)
